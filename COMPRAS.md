...
- **2026-06-30** — **Fix: filtro de Produto no Histórico ignorava entregas por divergência de caixa.** O campo `ordens_producao.produto` tem grafias mistas no banco (ex: **189** OPs `CALÇA DE MALHA` all-caps + **37** `Calça De Malha` title-case). O dropdown listava as duas como produtos distintos e o filtro comparava **com case** (`!== fProd`) → selecionar uma grafia escondia as entregas da outra (no caso reportado, as recém-fechadas com defeito: OPs 1508/1512/1513 de 24–25/jun). Fix **frontend**: o dropdown dedupa por chave `UPPERCASE` (value = chave normalizada, label = 1ª grafia vista) e o filtro compara case-insensitive (`(c.produto||'').trim().toUpperCase() !== fProd`). **Pendência durável (opcional):** normalizar `produto` no banco pra uma grafia única — a divergência também racha grupos no Consolidado por costureiro (agrupa por `produto`). O "DEF" no relato era coincidência (as recentes têm defeito).
