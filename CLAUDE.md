...
- **ecommerce.html**: `mvw_ecommerce_canal_dia` (KPIs + Waterfall) + **`mvw_ecommerce_sku_dia`** (09/06: lista de produtos + KPI Unidades no paint imediato; receita/qtd/cmv por sku_pai/dia/canal). `montarSkuGroups()` usa a MV até os itens carregarem (background → drawer/variantes/fee-dev-frete rateado + CMV real). Replica EXATAMENTE a lógica de itens (MAT-, custo herdado do pai, dedup, exclui cancelado+devolvido, só produtos ativos) — validado A/B (0 SKUs divergentes). DDL `sql/2026-06-09_mvw_ecommerce_sku_dia.sql`; refresh no cron de enriquecimento. Lista popula ~2,8s vs ~6,6s. Outras otimizações 09/06 no `buscarTudo`: **paginação paralela** (página 0 → se cheia, ondas paralelas) — corta o fetch serial de `vw_pedidos_full`; `fetchItensForIds` 5→8 batches paralelos. Load completo ~12s→~6s antes, lista ~2,8s depois.
